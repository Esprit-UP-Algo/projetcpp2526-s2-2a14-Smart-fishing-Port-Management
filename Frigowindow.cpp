...
void FrigoWindow::setupTable()
{
    ...
    table->setHorizontalHeaderLabels({"Référence", "Capacité", "Température", "Statut", "Poisson", "Occupation", "Actions"});
    ...
    table->setColumnWidth(0, 150);   // Référence
    table->setColumnWidth(1, 140);   // Capacité
    table->setColumnWidth(2, 140);   // Température
    table->setColumnWidth(3, 130);   // Statut
    table->setColumnWidth(4, 130);   // Poisson
    table->setColumnWidth(5, 140);   // Occupation
}
...
void FrigoWindow::populateTable(const QString& filterText)
{
    ...
    for (int i = 0; i < model->rowCount(); ++i) {
        ...
        // Map: ID(0), Ref(1), Cap(2), Temp(3), Stat(4), Type(5), Occ(6)
        int dbId = model->record(i).value(0).toInt();
        ...
        addItem(2, model->record(i).value(3).toString() + " °C"); // Température
        
        // Statut Badge
        table->setCellWidget(r, 3, createStatusBadge(model->record(i).value(4).toString()));
        
        addItem(4, model->record(i).value(5).toString()); // Type Poisson
        addItem(5, model->record(i).value(6).toString() + " %"); // Occupation
        
        // Actions
        table->setCellWidget(r, 6, createActionButtons(r)); // Use current row index for actions
    }
    delete model;
}
...
void FrigoWindow::onShowStatistics()
{
    ...
    QSqlQuery query("SELECT TYPEPOISSON, CAPACITE, STATUT FROM FRIGOS");
    while (query.next()) {
...
void FrigoWindow::onGeneratePDF()
{
    ...
    QSqlQuery query;
    query.prepare("SELECT REFERENCE, CAPACITE, TEMPERATURE, STATUT, TYPEPOISSON, OCCUPATION FROM FRIGOS WHERE IDFRIGO = :id");
    ...
    QString fish = query.value("TYPEPOISSON").toString();
...
void FrigoWindow::onAddFrigo()
{
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        // Use a default ID or handle differently if auto-inc, but FrigoModel needs int
        int newId = QDateTime::currentMSecsSinceEpoch() % 100000; 
        
        FrigoModel toSave(newId, newFrigoData.getRef(), newFrigoData.getCap(), 
                          newFrigoData.getTemp(), newFrigoData.getStat(), 
                          newFrigoData.getType(), newFrigoData.getOcc());
