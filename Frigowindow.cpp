...
{
    int maxId = 0;
    QSqlQuery query("SELECT IDFRIGO FROM GESTION_DES_FRIGOS");
    while (query.next()) {
        QString currentStr = query.value(0).toString();
        if (currentStr.startsWith("FR")) {
            int currentNum = currentStr.mid(2).toInt();
...
void FrigoWindow::onShowStatistics()
{
    ...
    QSqlQuery query("SELECT TYPE_POISSON, CAPACITE, STATUT FROM GESTION_DES_FRIGOS");
    while (query.next()) {
...
void FrigoWindow::onGeneratePDF()
{
    ...
    QSqlQuery query;
    query.prepare("SELECT REFERENCE, CAPACITE, TYPE_POISSON, STATUT, DATE_RESERVATION, TEMPERATURE, OCCUPATION FROM GESTION_DES_FRIGOS WHERE IDFRIGO = :id");
...
void FrigoWindow::onEditFrigo(int row)
{
    ...
    QSqlQuery query;
    query.prepare("SELECT * FROM GESTION_DES_FRIGOS WHERE IDFRIGO = :id");
