...
void BateauDialog::setupUi()
{
    ...
    // Âge du bateau
    QLabel* ageLabel = new QLabel("Âge du bateau (années)");
    ageLabel->setFont(labelFont);
    ageLabel->setStyleSheet("color: #2C3E50; margin-bottom: 5px;");
    formLayout->addWidget(ageLabel);

    ageBateauInput = new QLineEdit();
    ageBateauInput->setPlaceholderText("5");
    ageBateauInput->setFont(inputFont);
    ageBateauInput->setFixedHeight(50);
    ageBateauInput->setStyleSheet(getInputStyle());
    formLayout->addWidget(ageBateauInput);
...
void BateauDialog::populateFields()
{
    ...
    ageBateauInput->setText(bateauData->ageBateau);
...
Bateau BateauDialog::getData() const
{
    ...
    bateau.ageBateau = ageBateauInput->text();
