...
<<<<<<< HEAD
    ...
    langBtn = createNavButton("🌐", "FR / EN", false, false);
    connect(langBtn, &QPushButton::clicked, this, &MainWindow::toggleLanguage);
    navLayout->addWidget(langBtn);
...
<<<<<<< HEAD
    ...
    if (isDarkMode) {
        ...
        darkModeBtn->setText("🌙  Mode Sombre");
    }
...
void MainWindow::updateThemeRecursive(QWidget* widget, bool isDark)
{
    ...
    if (!style.isEmpty()) {
        if (isDark) {
            style.replace("#1e3a5f", "#F7FAFC", Qt::CaseInsensitive); // Primary Font
            style.replace("#2C3E50", "#F7FAFC", Qt::CaseInsensitive); // Title Font
            style.replace("#6B7280", "#A0AEC0", Qt::CaseInsensitive); // Subtitle Font
            style.replace("#1f2937", "#F7FAFC", Qt::CaseInsensitive); // Table/Normal Text Font
            style.replace("#0f172a", "#F7FAFC", Qt::CaseInsensitive); // Stats Value Font
            style.replace("#334155", "#F7FAFC", Qt::CaseInsensitive); // Secondary Dark Gray Font
            
            style.replace("#F0F4F8", "#1A202C", Qt::CaseInsensitive); // Main wrap
            style.replace("#f8fafc", "#1A202C", Qt::CaseInsensitive); // Alternative Main wrap
            style.replace("#F9FAFB", "#2D3748", Qt::CaseInsensitive); // Light Inputs

            style.replace("background-color: white", "background-color: #2D3748", Qt::CaseInsensitive); // Cards
            style.replace("background: white", "background: #2D3748", Qt::CaseInsensitive);             // Cards 2
            style.replace("#FFFFFF", "#2D3748", Qt::CaseInsensitive); // Generic Cards

            style.replace("#e2e8f0", "#4A5568", Qt::CaseInsensitive); // Base Borders
            style.replace("#d1d5db", "#4A5568", Qt::CaseInsensitive); // Lighter Grids
...
void MainWindow::translateRecursive(QWidget* widget, bool toEnglish)
{
    ...
    auto translateText = [toEnglish](QString& text) {
        ...
        for (auto it = dict.constBegin(); it != dict.constEnd(); ++it) {
            ...
            if (toEnglish && text.contains(fr)) {
                text.replace(fr, en);
            } else if (!toEnglish && text.contains(en)) {
                text.replace(en, fr);
            }
        }
    };
    ...
    if (QLabel* lbl = qobject_cast<QLabel*>(widget)) {
        ...
        QString text = line->placeholderText();
        translateText(text);
