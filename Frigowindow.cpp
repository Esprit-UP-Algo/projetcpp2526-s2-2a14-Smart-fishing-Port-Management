    ...
    : QMainWindow(parent)
{
    ...
    frigos.clear();
    QSqlQuery query("SELECT IDFRIGO, REFERENCE, CAPACITE, HUMIDITE, TEMPERATURE, STATUT, TYPEPOISSON FROM FRIGOS");
    while (query.next()) {
        Frigo f;
        f.id = QString("FR%1").arg(query.value(0).toInt(), 3, 10, QChar('0'));
        f.reference = query.value(1).toString();
        f.capacite = query.value(2).toString();
        f.humidite = query.value(3).toString();
        f.temperature = query.value(4).toString();
        f.statut = query.value(5).toString();
        f.poisson = query.value(6).toString();
        frigos.append(f);
    }
...
void FrigoWindow::setupTable()
{
    ...
    table->setHorizontalHeaderLabels({"Référence", "Capacité (Kg)", "Humidité (%)", "Température (°C)", "Statut", "Poisson", "Actions"});
...
void FrigoWindow::populateTable(const QString& filterText)
{
    ...
    for (int i = 0; i < frigos.size(); ++i) {
        ...
        if (!filterText.isEmpty()) {
            ...
            if (!frigo.reference.toLower().contains(searchLower) &&
    ...
    if (si == 1) std::sort(idx.begin(), idx.end(), [this](int a, int b) { return frigos[a].capacite.toDouble() < frigos[b].capacite.toDouble(); });
    else if (si == 2) std::sort(idx.begin(), idx.end(), [this](int a, int b) { return frigos[a].capacite.toDouble() > frigos[b].capacite.toDouble(); });
    else if (si == 3) std::sort(idx.begin(), idx.end(), [this](int a, int b) { return frigos[a].temperature.toDouble() < frigos[b].temperature.toDouble(); });
    else if (si == 4) std::sort(idx.begin(), idx.end(), [this](int a, int b) { return frigos[a].temperature.toDouble() > frigos[b].temperature.toDouble(); });
    ...
    for (int i : idx) {
        ...
        // Référence
        QTableWidgetItem* idItem = new QTableWidgetItem(frigo.reference);
...
{
    int maxId = 0;
    QSqlQuery query("SELECT MAX(IDFRIGO) FROM FRIGOS");
    if (query.next()) {
...
void FrigoWindow::onShowStatistics()
{
    ...
    for (const Frigo& f : frigos) {
        typeCount[f.poisson]++;
        typeCapacity[f.poisson] += f.capacite.toDouble();
        statusCount[f.statut]++;
    }
...
void FrigoWindow::onGeneratePDF()
{
    ...
    int frigoIndex = item->data(Qt::UserRole).toInt();
    if (frigoIndex < 0 || frigoIndex >= frigos.size()) return;

    const Frigo& f = frigos[frigoIndex];

    QString path = QFileDialog::getSaveFileName(this, "Exporter Reçu PDF", "recu_" + f.reference + ".pdf", "PDF (*.pdf)");
    ...
    p.drawText(20, y, "Référence du Frigo : " + f.reference); y += 40;
    p.drawText(20, y, "Capacité : " + f.capacite + " Kg"); y += 40;
    p.drawText(20, y, "Humidité / Température : " + f.humidite + " % / " + f.temperature + " °C"); y += 40;
    p.drawText(20, y, "Type de poisson : " + f.poisson); y += 40;
    p.drawText(20, y, "Statut actuel : " + f.statut); y += 40;

    // Simulation d'un prix de stockage
    double pricePerKg = 2.5;
    double total = f.capacite.toDouble() * pricePerKg;
...
void FrigoWindow::onAddFrigo()
{
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        QSqlQuery query;
        query.prepare("INSERT INTO FRIGOS (IDFRIGO, REFERENCE, CAPACITE, HUMIDITE, TEMPERATURE, STATUT, TYPEPOISSON) "
                      "VALUES (:id, :ref, :cap, :hum, :temp, :status, :fish)");
        int numId = newFrigo.id.mid(2).toInt();
        query.bindValue(":id", numId);
        query.bindValue(":ref", newFrigo.reference);
        query.bindValue(":cap", newFrigo.capacite.toDouble());
        query.bindValue(":hum", newFrigo.humidite.toDouble());
        query.bindValue(":temp", newFrigo.temperature.toDouble());
        query.bindValue(":status", newFrigo.statut);
        query.bindValue(":fish", newFrigo.poisson);
        ...
        if (query.exec()) {
            ...
            QMessageBox::critical(this, "Erreur", "L'ajout a échoué: " + query.lastError().text());
        }
    }
}
...
void FrigoWindow::onEditFrigo(int row)
{
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        QSqlQuery query;
        query.prepare("UPDATE FRIGOS SET REFERENCE=:ref, CAPACITE=:cap, HUMIDITE=:hum, TEMPERATURE=:temp, "
                      "STATUT=:status, TYPEPOISSON=:fish WHERE IDFRIGO=:id");
        int numId = updatedFrigo.id.mid(2).toInt();
        query.bindValue(":id", numId);
        query.bindValue(":ref", updatedFrigo.reference);
        query.bindValue(":cap", updatedFrigo.capacite.toDouble());
        query.bindValue(":hum", updatedFrigo.humidite.toDouble());
        query.bindValue(":temp", updatedFrigo.temperature.toDouble());
        query.bindValue(":status", updatedFrigo.statut);
        query.bindValue(":fish", updatedFrigo.poisson);
...
void FrigoWindow::onDeleteFrigo(int row)
{
    ...
    if (reply == QMessageBox::Yes) {
        ...
        int numId = frigos[row].id.mid(2).toInt();
        query.bindValue(":id", numId);
        ...
        if (query.exec()) {
            ...
            QMessageBox::critical(this, "Erreur", "La suppression a échoué: " + query.lastError().text());
        }
    }
}
