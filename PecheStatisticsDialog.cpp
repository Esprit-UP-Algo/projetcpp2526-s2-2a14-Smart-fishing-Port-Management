    ...
    : QDialog(parent)
{
    ...
    setupUi(speciesCount, weightBySpecies, avgWeightByBoat);
}
                                    ...
                                    const QMap<QString, double>& avgWeightByBoat)
{
    ...
    makeMetric("🛥️", "Moyenne/Bateau", QString::number(avgWeightByBoat.size() > 0 ? totalWeight / avgWeightByBoat.size() : 0, 'f', 1) + " Kg", "#DBEAFE", "#1E40AF");
    ...
    PechePieChartWidget* chartSpecies = new PechePieChartWidget("Lots par Espèce", speciesCount);
    PechePieChartWidget* chartWeight = new PechePieChartWidget("Poids par Espèce", weightBySpecies);
    PechePieChartWidget* chartBoat = new PechePieChartWidget("Poids Moyen par Bateau", avgWeightByBoat);

    chartsRow->addWidget(makeChartFrame(chartSpecies), 1);
    chartsRow->addWidget(makeChartFrame(chartWeight), 1);
    chartsRow->addWidget(makeChartFrame(chartBoat), 1);
