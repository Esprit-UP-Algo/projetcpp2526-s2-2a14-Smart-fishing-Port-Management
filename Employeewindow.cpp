...
void EmployeeWindow::setupTable()
{
    ...
    table->setHorizontalHeaderLabels({"Prénom", "Nom", "Position", "Salaire", "Date", "Statut", "Actions"});
    ...
    table->setColumnWidth(0, 120);  // Prénom
    table->setColumnWidth(1, 120);  // Nom
    table->setColumnWidth(2, 120);  // Position
    table->setColumnWidth(3, 100);  // Salaire
    table->setColumnWidth(4, 110);  // Date
    table->setColumnWidth(5, 100);  // Statut
}
...
void EmployeeWindow::populateTable(const QString& filterText)
{
    ...
    for (int i = 0; i < employees.size(); ++i) {
        ...
        // Statut
        table->setCellWidget(row, 5, createStatusBadge(emp.status));
