    ...
    : QDialog(parent), m_age(age), m_moisMaintenance(moisMaintenance), m_frequence(frequence), m_etat(etat)
{
    ...
    trayIcon->setIcon(QIcon("C:/images/logo.png"));
    trayIcon->show();
...
void PredictMaintenanceDialog::calculateScore()
{
    ...
    if (score >= 5.0 && trayIcon->isSystemTrayAvailable()) {
        QSystemTrayIcon::MessageIcon iconType = (score > 7.0) ? QSystemTrayIcon::Critical : QSystemTrayIcon::Warning;
        trayIcon->showMessage("Alerte " + QString((score > 7.0) ? "URGENTE" : "de SURVEILLANCE"), 
                              QString("Bateau nécessitant une action.\nDécision: %1\nScore final : %2")
                              .arg(decision)
                              .arg(score, 0, 'f', 2),
                              iconType,
                              6000);
