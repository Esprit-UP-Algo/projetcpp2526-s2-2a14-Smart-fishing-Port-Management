...
{
    QSqlQuery query;
    query.prepare("INSERT INTO GESTION_DES_FRIGOS (IDFRIGO, REFERENCE, CAPACITE, TYPE_POISSON, STATUT, DATE_RESERVATION, TEMPERATURE, OCCUPATION) "
    ...
    if (!query.exec()) {
        ...
        query.clear();
        QSqlQuery fb;
        fb.prepare("INSERT INTO GESTION_DES_FRIGOS (IDFRIGO, REFERENCE, CAPACITE, TYPE_POISSON, STATUT, DATE_RESERVATION, TEMPERATURE, OCCUPATION) "
                   "VALUES (:id, :ref, :cap, :type, :stat, TO_DATE(:dateRes, 'DD/MM/YYYY'), :temp, :occ)");
...
QSqlQueryModel* FrigoModel::afficher()
{
    ...
    model->setQuery("SELECT IDFRIGO, REFERENCE, CAPACITE, TYPE_POISSON, STATUT, DATE_RESERVATION, TEMPERATURE, OCCUPATION FROM GESTION_DES_FRIGOS");
...
{
    QSqlQuery query;
    query.prepare("DELETE FROM GESTION_DES_FRIGOS WHERE IDFRIGO = :id");
...
{
    QSqlQuery query;
    query.prepare("UPDATE GESTION_DES_FRIGOS SET REFERENCE=:ref, CAPACITE=:cap, TYPE_POISSON=:type, "
...
QSqlQueryModel* FrigoModel::rechercher(QString val)
{
    QSqlQueryModel* model = new QSqlQueryModel();
    QSqlQuery query;
    query.prepare("SELECT IDFRIGO, REFERENCE, CAPACITE, TYPE_POISSON, STATUT, DATE_RESERVATION, TEMPERATURE, OCCUPATION FROM GESTION_DES_FRIGOS WHERE REFERENCE LIKE :v OR STATUT LIKE :v OR TYPE_POISSON LIKE :v");
...
{
    QSqlQueryModel* model = new QSqlQueryModel();
    QString qStr = QString("SELECT IDFRIGO, REFERENCE, CAPACITE, TYPE_POISSON, STATUT, DATE_RESERVATION, TEMPERATURE, OCCUPATION FROM GESTION_DES_FRIGOS ORDER BY %1 %2").arg(critere, ordre);
    model->setQuery(qStr);
    return model;
}
