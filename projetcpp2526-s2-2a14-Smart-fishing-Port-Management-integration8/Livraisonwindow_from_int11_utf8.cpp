...
QFrame* LivraisonWindow::createHeader()
{
    ...
    QPushButton* statsBtn = makeBtn("­ƒôè  Statistiques", "#7C3AED", "#6D28D9");
    QPushButton* addBtn   = makeBtn("Ô×ò  Nouvelle Livraison", "#2563EB", "#1D4ED8");
...
QFrame* LivraisonWindow::createToolbar()
{
    ...
    sortCombo->addItems({"D├®faut", "Date (R├®cent)", "Date (Ancien)", "Prix Ôåæ", "Prix Ôåô"});
...
QFrame* LivraisonWindow::createStatsArea()
{
    ...
    QLabel* totalIcon = new QLabel("­ƒôª");
    ...
    QLabel* effTitle = new QLabel("Efficacit├® (Livr├®es)");
...
void LivraisonWindow::updateStats()
{
    ...
    // Scoping the query ensures the handle is released immediately
    {
        ...
        while (query.next()) {
            total++;
            if (query.value(0).toString() == "Livr├®") {
                delivered++;
            }
        }
    }
...
QWidget* LivraisonWindow::createStatusBadge(const QString& status)
{
    ...
    } else if (status == "Annul├®" || status == "Canceled") {
...
QWidget* LivraisonWindow::createActionButtons(int row)
{
    ...
    QPushButton* editBtn = new QPushButton("Ô£Å´©Å");
    QPushButton* deleteBtn = new QPushButton("­ƒùæ´©Å");
    QPushButton* pdfBtn = new QPushButton("­ƒôä");
...
void LivraisonWindow::onExportPDF(int row)
{
    ...
    painter.drawText(QRect(0, 50, 5000, 100), Qt::AlignCenter, "RE├çU DE LIVRAISON");
    ...
    QMessageBox::information(this, "Export PDF", "Le re├ºu de livraison a ├®t├® export├® avec succ├¿s !");
}
...
void LivraisonWindow::onExportAllPDF()
{
    ...
    QMessageBox::information(this, "Export PDF", "Le rapport global a ├®t├® export├® avec succ├¿s !");
}
