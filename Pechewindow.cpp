...
void PecheWindow::setupTable()
{
    ...
    table->setHorizontalHeaderLabels({
        "Référence", "Catégorie", "Quantité (Kg)", "Date", "Bateau", "Frigo", "Actions"
    });
...
void PecheWindow::populateTable(const QString& filterText)
{
    ...
    for (int i = 0; i < model->rowCount(); ++i) {
        ...
        // Stocker les IDs pour l'édition
        QTableWidgetItem* bItem = it(boat);
        bItem->setData(Qt::UserRole, boat);
        table->setItem(r, 4, bItem);
        
        QTableWidgetItem* fItem = it(fridge);
        fItem->setData(Qt::UserRole, fridge);
...
QString PecheWindow::generatePecheId() {
    QSqlQuery q;
    q.exec("SELECT MAX(IDLOT) FROM PECHES");
    int mx = 0;
    if (q.next()) mx = q.value(0).toInt();
...
void PecheWindow::onGeneratePDF()
{
    ...
    // ── Query & title ──────────────────────────────────────────
    QString queryStr  = "SELECT * FROM PECHES";
    ...
    if (dlg.exportType() == PecheExportDialog::ByBoat) {
        queryStr = QString("SELECT * FROM PECHES WHERE IDBATEAU = '%1'").arg(dlg.selectedBoat());
        rpTitle  = QString("Rapport de Pêche — Bateau %1").arg(dlg.selectedBoat());
        rpSub    = QString("Captures enregistrées pour le bateau %1").arg(dlg.selectedBoat());
    } else if (dlg.exportType() == PecheExportDialog::ByDate) {
        queryStr = QString("SELECT * FROM PECHES WHERE DATECAPTURE BETWEEN "
    ...
    QList<int> cw = { cW*22/100, cW*26/100, cW*24/100, cW*28/100 };
    QStringList hd = { "Référence", "Catégorie", "Quantité (Kg)", "Date" };
    const int rH = 40;
